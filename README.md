...
*\* Cost model: Missed failure (FN) = \$50,000 (unplanned downtime); False alarm (FP) = \$5,000 (unnecessary inspection).*  
*Test set: 20 held‑out engines, 3,852 cycles (620 at‑risk).*

### Key Insight
The naive baselines catch almost all failures, but **flood operators with false alarms** (750–1,030 per test set) — making them impractical.  
The **rolling degraded‑state counter** achieves the **best deployment‑ready balance**: high recall (94.0%) with moderate false alarms (230), yielding **~$1M less operational cost** than the Random Forest.
...
> **Conclusion:** Even after removing all forms of look‑ahead bias, the single rolling‑count feature **consistently and significantly outperforms** the 21‑sensor Random Forest on the critical at‑risk recall metric.
...
### 1. Download the data
Place `train_FD001.txt` from the [NASA C‑MAPSS dataset](https://data.nasa.gov/docs/legacy/CMAPSSData.zip) into a `data/` folder.
