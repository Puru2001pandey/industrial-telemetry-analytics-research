...
\fancyhead[C]{\small\textit{Published on Zenodo -- September 2026}}
\fancyfoot[C]{\thepage}
...
recall rather than RMSE, accuracy, or overall weighted F1.
...
The narrow question---does one number derived from domain knowledge of degradation
dynamics beat 21 raw channels on the metric that deployment actually requires?---does
not appear to have been posed directly in prior work.
...
\noindent\textbf{Note on look-ahead.}
The percentile $Q_{0.70}$ is computed over the \emph{full} run-to-failure trajectory
of each engine, including cycles that fall in the at-risk window.
For the training engines this introduces a mild look-ahead bias: the threshold
is influenced by the degraded-phase readings it is intended to detect.
In a live deployment the threshold would instead be estimated from historical
healthy-phase data; post-hoc analysis on archived run-to-failure logs---the setting
evaluated here---is the scenario where full-trajectory percentiles are appropriate.
This limitation is acknowledged explicitly in Section~\ref{sec:sensitivity}.
...
To assess whether these results depend on the specific 80/20 split, the experiment
was repeated 30 times with seeds $0$--$29$.
The LR achieved a mean at-risk recall of $\mathbf{0.957 \pm 0.018}$ versus
$\mathbf{0.916 \pm 0.026}$ for the RF (paired $t$-test: $t = 9.98$, $p < 0.0001$).
The LR exceeded RF recall in 29 of 30 splits; in the one tied split (seed 11), both
achieved 0.977.
The single-split result in Table~\ref{tab:results} (seed 42) is representative of this
distribution.
...
LR misses 15 at-risk cycles; RF misses 74.}
\label{tab:cm}
\vspace{4pt}
\small
...
\midrule
Actual 0 & 3{,}335 & 336 & 3{,}561 & 110 \\
Actual 1 & 15 & 605 & 74 & 546 \\
\bottomrule
\end{tabular}
\end{table}
...
The Random Forest recall remains constant across variations in $w$ and $\tau$ because
its input features (the raw sensor channels) are independent of the windowing
parameters; this provides a fixed baseline against which the LR's parameter sensitivity
is measured.
...
model.
But 74 undetected imminent failures versus 15 is not a small difference in a
...
detections alone, against $15 \times \$50{,}000 = \$750{,}000$ for the logistic
regression.
The logistic regression generates more false alarms (336 vs.\ 110), adding $336 \times
\$5{,}000 = \$1{,}680{,}000$ versus $110 \times \$5{,}000 = \$550{,}000$.
Net expected cost: LR \$2.43M vs.\ RF \$4.25M---the simpler model is preferable under
...
The results suggest that evaluation metric choice is a design decision with operational
consequences, not a secondary concern.
Deploying a model that maximises weighted F1 on a 14\% minority-class dataset is likely
to deploy a model that performs well on paper and misses failures in practice.
The appropriate metric should be determined by the cost structure of the deployment
context, not by which number looks best in a comparison table.

The full pipeline is available on GitHub under the MIT licence.
Clone the repository and run the \texttt{run\_reproduction.ipynb} notebook to reproduce
...
\texttt{run\_reproduction.ipynb}.
The notebook will raise a \texttt{FileNotFoundError} if the data file is absent.
